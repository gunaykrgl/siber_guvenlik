...
Github Repository link: [gunaykrgl/siber_guvenlik](https://github.com/gunaykrgl/siber_guvenlik/)
...
The attacker may sever a cable at a specific location and deploy a device, such as a Raspberry Pi, under their control. This device could be configured to intercept and duplicate incoming packets, sending an identical copy both to the intended destination and the attacker's device. Through this method, the attacker gains the ability to divert the network traffic through their own device, allowing for unauthorized access and potential interception of sensitive information (Compromising CONFIDENTIALITY).
    ...
    **Counter-Strategy**
    ...
    - Utilizing TLS (Transport layer Security) encryption can impede an attacker's capacity to gather sensitive information from intercepted data packets. 
        ...
        TLS works as follows [(4)]:

        1. The service that wants to implement TLS must get a TLS certificate

        2. When the service gets TLS certificate, it is given a pair of private and public keys. The produced public key can be used to verify the hash was generated by the correct private key without needing to know the private key. As two different keys are used for encrypting and verifying, this method of cryptography is called "Asymmetric cryptography". [(5)]
        ...
        3. When the user navigates to the website that uses TLS, the TLS handshake begins between the user's device and the web server. 
            ...
            During the TLS handshake the following steps are taken [(6)]:
        ...
        4. The subsequent data exhanged between the client and server are encrypted and decrypted with the "session key" that was created during TLS handshake. As the same key is used for both encryption and decryption, it is called "symmetric encryption". 
        (Note: The reason, symmetric encryption is used is it is faster than asymmetric encryption while maintaining data security during the transmission)
...
(Compromising AVAILABILITY)
    ...
    **Common types of DDoS attacks [(8)]**
...
A fake DNS attack commonly involves the attacker positioning themselves between the user and the DNS server, redirecting the user to a malicious website designed to mimic the appearance of the intended site. In this type of attack, a typical user may not readily discern that they have landed on a fraudulent website since the URL in the browser's top bar still displays the correct textual address. Consequently, users may unknowingly provide sensitive information to the attackers. (could be used to compromise INTEGRITY OR CONFIDENTIALITY)
    ...
    **Counter-Measure**
    ...
    - By using Domain Name System Security Extensions (DNSSEC)[(9)], the communication with DNSSEC is signed. Therefore, the user's computer can verify whether the DNS record sent by the server is from the registrar itself.
...
4. [Transport layer security, cloudflare](https://www.cloudflare.com/learning/ssl/transport-layer-security-tls/)

5. [How does public key encryption work](https://www.cloudflare.com/learning/ssl/how-does-public-key-encryption-work/)

6. [Steps in a TLS handshake](https://www.cloudflare.com/learning/ssl/what-happens-in-a-tls-handshake/)

7. [IDS, firewall and Antivirus: What you need to have installed, welivesecurity by eset](https://www.welivesecurity.com/2015/04/30/ids-firewall-antivirus-need-installed/)

8. [About DDoS ,Cloudflare](https://www.cloudflare.com/learning/ddos/what-is-a-ddos-attack/)

9. [DNSSEC, imperva](https://www.imperva.com/learn/application-security/dnssec/)